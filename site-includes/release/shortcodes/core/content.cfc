...
dynamically in the Web Admin CMS
...
to where it was entered by the editor in the Web Admin.
...
<!--- for multiple contentFors - CM / WE --->
	...
	$('###local.index.UUID#').addClass('for-#local.index.for#-placeholder staticFor').clone().removeAttr('class').appendTo($container);
