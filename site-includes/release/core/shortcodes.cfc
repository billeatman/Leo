...
<cffunction name="runCodes" access="public" returntype="string" output="false">
    ...
    <cfif len(c.method) gt 0>
    	<cfif arguments.clearCodes EQ true>
    		<cfset c.content = "">
    	<cfelseif isNumeric(c.method)>
    		<cfset c.content = "[#c.method#]">
    	<cfelse>
...
<cffunction name="createShortCode" access="public" returntype="ANY">
	...
	<cftry>
		...
		<cfinvoke component="#local.code#" method="runCode" argumentcollection="#c.args#" returnvariable="c.content">
		   	<cfinvokeargument name="s_content" value="#c.content#">
		</cfinvoke>
			
		<cfcatch>
<!---		<cfdump var="#arguments.site_config.cfMappedPath#.#arguments.site_config.sitepath#.shortcodes.#variables.override_directory#.#c.method#">
				<cfdump var="#cfcatch#">
				<cfabort>
				<cfrethrow> --->
					
		<!--- 	<cfset c.content = cfcatch.message & cfcatch.errorcode> --->
<!---				<cfif arguments.site_config.displayErrors EQ false> --->
<!---				<cfthrow detail="ERROR: #cfcatch.message# #c.method#"> --->
<!---				</cfif> --->
<!--- 				<cfset c.content = "ERROR: #cfcatch.message# #c.method#"> --->
				...
				<cfelseif structKeyExists(cfcatch, "missingFileName")>
					<cfrethrow>
					<cfset c.content = "">  <!--- mid(arguments.html, 3802, 15)> --->
				<cfelse>
					<cfrethrow>
				</cfif>
		</cfcatch>	
