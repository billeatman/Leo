...
<cffunction name="getSitemapXML" access="remote" output="false" returntype="any" returnFormat="plain"> 
    ...
    <cfinvoke component="#baseCFCPath#.core.pageHelper" method="getBasePath" returnvariable="basepath" />
    ...
    select ApprovedDate, ID, SSL from sitePublic
    </cfquery>
    ...
    <cfif isNull(sitemap) OR site_config.useCache NEQ true>
        ...
        <cfinclude template="sitemapcustom.cfm">
        ...
        <cfloop query="qPages">
            ...
            <cfif site_config.URLrewriter EQ true>
                <!--- SEO rewritter logic --->
                <cfinvoke component="#baseCFCPath#.core.pageHelper" method="pageIdToURL" returnvariable="urlPath">
            ...
            <cfquery datasource="#site_config.datasource#" name="qHistory">
                select count(*) as count from pageAction where [action] = 'approved' and [id] = '#qPages.id#' and actionDate > <cfqueryparam cfsqltype="cf_sql_date" value="#DateAdd('yyyy', -1, now())#">
            </cfquery>
